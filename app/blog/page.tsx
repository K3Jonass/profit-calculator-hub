  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-12 md:py-16">
      ...
      <section className="mt-12 space-y-12">
        ...
        {BLOG_HUB_CATEGORIES.map((category) => {
          const posts = sections.get(category.key) || [];
          return (
            <section key={category.key} className="space-y-5">
              <h3 className="text-2xl font-semibold tracking-tight text-slate-900 dark:text-slate-100">{category.label}</h3>
                ...
                {posts.length ? (
                  posts.map((post) => (
                    <Link key={post.href} href={withLocale(post.href, locale)} className="hover-lift group rounded-3xl border-soft surface-card p-6">
                      <div className={BLOG_BADGE_CLASSNAME}>{category.label}</div>

                      <h4 className="mb-3 text-2xl font-semibold leading-snug text-slate-900 transition group-hover:text-blue-700 dark:text-slate-100 dark:group-hover:text-blue-300">
                        {post.title}
                      </h4>
                  ...
                  <div className="rounded-3xl border border-dashed border-slate-300 bg-slate-50 p-6 dark:border-slate-700 dark:bg-slate-900/60">
                    <p className="text-sm text-slate-600 dark:text-slate-300">{EMPTY_CATEGORY_COPY}</p>
                  </div>
                )}
              </div>
            </section>
        ...
        {(sections.get("uncategorized") || []).length > 0 ? (
          <section className="space-y-5">
            <h3 className="text-2xl font-semibold tracking-tight text-slate-900 dark:text-slate-100">
              {UNCATEGORIZED_SECTION_LABEL}
            </h3>
            <div className="motion-stagger grid gap-6 md:grid-cols-2 xl:grid-cols-3">
              {(sections.get("uncategorized") || []).map((post) => (
                <Link key={post.href} href={withLocale(post.href, locale)} className="hover-lift group rounded-3xl border-soft surface-card p-6">
                  <div className={UNCATEGORIZED_BADGE_CLASSNAME}>{UNCATEGORIZED_SECTION_LABEL}</div>
