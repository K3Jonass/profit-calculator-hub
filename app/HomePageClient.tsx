...
const categories = [
  {
    ...
    links: [
      ...
      { href: "/calculators/dropshipping-profit", label: "Dropshipping Profit Calculator" },
      {
        ...
        label: "Breakeven Calculator for Product Pricing",
      },
    ],
  },
  {
    ...
    links: [
      { href: "/calculators/saas-mrr", label: "SaaS MRR Calculator" },
      {
        ...
        label: "Subscription Leak Calculator",
      },
  ...
  },
  {
    ...
    links: [
      { href: "/calculators/freelance-rate", label: "Freelance Rate Calculator" },
      {
        ...
        label: "Freelance Project Profit Analyzer",
      },
    ...
    <main className="mx-auto max-w-7xl px-4 py-10 md:px-6 md:py-14">
      <section className="motion-fade-up rounded-[2rem] border border-slate-200 bg-slate-950 px-6 py-14 text-white shadow-sm md:px-10 md:py-16">
        ...
        <p className="mt-5 max-w-3xl text-lg leading-8 text-slate-300">
          Use ProfitHub to calculate profit margins, MRR, breakeven targets,
          subscription leakage, revenue share payouts, and decision impact with
          practical tools designed for operators.
        </p>
      ...
      <section className="motion-fade-up mt-14">
        ...
        <div className="mt-6 grid gap-5 md:grid-cols-3">
          {categories.map((category) => (
            <article
              key={category.title}
              className="hover-lift rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"
            >
              ...
              <ul className="mt-4 space-y-2 text-sm">
                {category.links.map((link) => (
                  <li key={link.href}>
                    <Link
                      href={link.href}
                      className="font-medium text-slate-900 underline decoration-slate-300 underline-offset-4 hover:text-blue-700"
                    >
                      {link.label}
                    </Link>
                  </li>
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {popularCalculators.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="hover-lift rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"
            >
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2">
          {useCases.map((useCase) => (
            <Link
              key={useCase.title}
              href={useCase.href}
              className="hover-lift rounded-2xl border border-slate-200 bg-white p-6 shadow-sm hover:border-blue-200 hover:shadow-md"
            >
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {latestGuides.map((post) => (
            <Link
              key={post.href}
              href={post.href}
              className="hover-lift rounded-2xl border border-slate-200 bg-white p-5 shadow-sm hover:-translate-y-0.5 hover:shadow-md"
            >
      ...
      <section className="motion-fade-up mt-14">
        <h2 className="text-3xl font-bold tracking-tight text-slate-900">
          Featured tools for growth decisions
        </h2>
        <div className="mt-6 grid gap-4 md:grid-cols-2">
      ...
      <section className="motion-fade-up mt-14 rounded-3xl border border-blue-200 bg-blue-50 px-6 py-10">
        ...
        <p className="mt-3 max-w-3xl text-slate-700">
          Pick one calculator, run your numbers, then use linked guides to
          improve pricing, retention, and profitability decisions this week.
        </p>
